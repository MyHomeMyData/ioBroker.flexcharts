...
class ExtensionFlexcharts {
    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        // Forward ioBroker state changes to all registered SSE clients
        this.adapter.on('stateChange', id => {
            const clients = this.sseClients.get(id);
            if (clients && clients.size > 0) {
                this.adapter.log.debug(`SSE: state ${id} changed, notifying ${clients.size} client(s)`);
    ...
    sseEvents(req, res) {
        ...
        if (!this.sseClients.has(stateId)) {
            ...
            this.adapter.log.debug(`SSE: subscribed to state ${stateId}`);
        }
        ...
        req.on('close', () => {
            ...
            if (clients) {
                ...
                if (clients.size === 0) {
                    ...
                    this.adapter.log.debug(`SSE: unsubscribed from state ${stateId}`);
                }
            }
        });
    }
