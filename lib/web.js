...
class ExtensionFlexcharts {
    ...
    constructor(server, webSettings, adapter, instanceSettings, app) {
        ...
        this.rxPreInit = /\/\* preInitFunctions\(\) \*\//; // Regular Expression to replace '/* preInitFunctions() */' in echarts.html
        this.rxPreInitPlain = '/* preInitFunctions() */'; // Plain version of replacement text in echarts.html
        // preInitFunctions() runs BEFORE setTheme() is first applied (see echarts.html): code that must be in
        // place before the chart's theme is set — e.g. echarts.registerTheme() — belongs here, not in
        // userFunctions(). setTheme() replays only the very first setOption() call it sees ("recreate"), so
        // anything relying on setOption() (e.g. registerMap() + setOption() event handlers) still belongs in
        // userFunctions(), which runs after the theme has already been applied once.
        ...
        // SSE client registry: stateId -> Map<res, clientState>
        this.sseClients = new Map();
        // SSE polling intervals: stateId -> interval handle
        this.ssePollingIntervals = new Map();
        // SSE last push timestamps: stateId -> Date.now() — persists across client reconnects for throttle
        this.sseLastPushTime = new Map();
    ...
    /**
     * Like applyUserFunctions(), but targets the preInitFunctions() slot in echarts.html, which runs
     * BEFORE the chart's theme is first applied via setTheme(). Use this for code that setTheme() itself
     * depends on, such as echarts.registerTheme() — see the comment on this.rxPreInit for details.
     *
     * @param {Buffer} content html content
     * @param {string} preInitFct Definition of pre-init function
     */
    applyPreInitFunctions(content, preInitFct) {
        const r = preInitFct.replaceAll(/\$/g, '$$$$'); // Mask '$' (by replacing it with $$) to avoid evaluation in following replacement action
        if (r.length > 0) {
            content = Buffer.from(content.toString().replace(this.rxPreInit, r + this.rxPreInitPlain));
        }
        return content;
    }
        ...
        if (typeof themeDef[0] == 'string' && typeof themeDef[1] == 'string') {
            ...
            if (r.length > 0) {
                if (r.includes('registerTheme')) {
                    ...
                    content = Buffer.from(content.toString().replace(this.rxPreInit, f + this.rxPreInitPlain));
                }
            }
        } else {
     ...
     * [>0] => array[2]: theme definition [name, stringifiedThemeJson] — applied via applyTheme(), which
     *         inserts into the preInitFunctions() slot so the theme is registered before setTheme() runs
     *
     * @param {Buffer} content html content
     ...
     *
     * The &sse parameter controls throttle and ack filter (parsed from the forwarded chart URL):
     *   &sse           => refresh=5s, ack=any
     *   &sse=<n>       => refresh=max(5,n) seconds, ack=any
     *   &sse=<json>    => refresh and/or ack from JSON object, e.g. {"refresh":10,"ack":true}
     *
    ...
    sseEvents(req, res) {
        ...
        // Parse &sse parameter: number (seconds), JSON object {"refresh":<n>,"ack":true|false}, or empty
        const SSE_MIN_REFRESH_MS = 5000;
        const sseRaw = query.sse || '';
        let refreshMs = SSE_MIN_REFRESH_MS;
        let ackFilter = null; // null = accept any ack value
        if (sseRaw !== '') {
            const num = Number(sseRaw);
            if (!isNaN(num)) {
                refreshMs = Math.max(SSE_MIN_REFRESH_MS, num * 1000);
            } else {
                try {
                    const json = JSON.parse(sseRaw);
                    refreshMs = Math.max(SSE_MIN_REFRESH_MS, (json.refresh || 5) * 1000);
                    if (json.ack !== undefined) {
                        ackFilter = Boolean(json.ack);
                    }
                } catch {
                    this.adapter.log.warn(`SSE: invalid &sse value "${sseRaw}", using defaults`);
                }
            }
        }
        ...
        // Per-client state for throttle and ack filtering
        const clientState = {
            lastTs: 0, // last seen state timestamp
            pendingChange: false, // state changed but refresh delay not yet elapsed
            lastPushTime: this.sseLastPushTime.get(stateId) || 0, // survives page reload
            refreshMs,
            ackFilter,
        };

        // Register client; start polling when first client subscribes to this stateId.
        // sseClients: stateId -> Map<res, clientState>
        // Each client tracks its own throttle and ack state; the poll interval is shared.
        if (!this.sseClients.has(stateId)) {
            this.sseClients.set(stateId, new Map());
            ...
            const poll = setInterval(async () => {
                ...
                try {
                    ...
                    return; // ignore transient DB errors
                }
                const ts = state ? state.ts : 0;
                const now = Date.now();
                for (const [clientRes, cs] of clients) {
                    // Detect state change
                    if (ts > cs.lastTs && cs.lastTs > 0) {
                        // Apply ack filter: mark change as pending only if ack matches
                        if (cs.ackFilter === null || (state && state.ack === cs.ackFilter)) {
                            cs.pendingChange = true;
                        }
                    }
                    cs.lastTs = ts; // always advance baseline, even if filtered
                    // Push SSE event when pending and throttle delay has elapsed
                    if (cs.pendingChange && now - cs.lastPushTime >= cs.refreshMs) {
                        try {
                            clientRes.write('data: {}\n\n');
                            if (typeof clientRes.flush === 'function') {
                                clientRes.flush();
                            }
                            cs.pendingChange = false;
                            cs.lastPushTime = now;
                            this.sseLastPushTime.set(stateId, now); // persist across reconnects
                            this.adapter.log.debug(
                                `SSE: pushed reload for ${stateId} (refresh=${cs.refreshMs}ms, ack=${cs.ackFilter})`,
                            );
                        } catch {
        ...
        this.sseClients.get(stateId).set(res, clientState);
        this.adapter.log.debug(
            `SSE: client connected for ${stateId}, total: ${this.sseClients.get(stateId).size}, refresh=${refreshMs}ms, ack=${ackFilter}`,
        );
    ...
    /**
     * Data endpoint: returns only the chart option as plain text for SSE-triggered setOption updates.
     * Activated by URL /flexcharts/data?<same params as echarts.html request>
     *
     * @param {object} req Express request object
     * @param {object} res Express response object
     */
    dataEndpoint(req, res) {
        ...
        switch (query.source) {
                ...
                this.adapter.sendTo(
                    ...
                    result => {
                        if (result && result.error) {
                            res.writeHead(500);
                            res.end(result.error, 'utf-8');
                        } else if (result && result == 'Error: Timeout exceeded') {
                            res.writeHead(504);
                            res.end('Timeout', 'utf-8');
                        } else if (Array.isArray(result)) {
                            sendOption(result[0]);
                        } else if (typeof result === 'string') {
                            sendOption(result);
                        } else {
                            sendOption(JSON.stringify(result));
                        }
                    },
                    { timeout: 2000 },
                );
            ...
            case 'state': {
                ...
                this.adapter.getForeignState(query.id, (_error, result) => {
                    if (result && result.val !== undefined && result.val !== null) {
                        if (typeof result.val === 'string') {
                            ...
                            sendOption(result.val);
                        } else {
                            sendOption(JSON.stringify(result.val));
                        }
                    } else {
                        res.writeHead(404);
                        res.end(`Could not read state id ${query.id}`, 'utf-8');
                    }
                });
                break;
            }
            default: {
                sendOption(JSON.stringify(demoChart()));
                break;
            }
        }
    }
    ...
    eCharts(req, res) {
        ...
        // Route data-only requests to dedicated handler (used by SSE setOption updates)
        if (req.url && req.url.split('?')[0] === '/data') {
            this.dataEndpoint(req, res);
            return;
        }
        ...
        fs.readFile(filePath, (error, content) => {
            ...
            const applyCommonOptions = cnt => {
                ...
                if (queryThemeV5) {
                    cnt = this.applyPreInitFunctions(cnt, this.strThemeV5);
                    cnt = this.applyPreInitFunctions(cnt, this.strThemeV5Dark);
                }
                return cnt;
            };
