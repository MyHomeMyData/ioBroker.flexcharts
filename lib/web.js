...
class ExtensionFlexcharts {
    ...
    // Select MIME-type based on file extension
    ...
    eCharts(req, res) {
        ...
        fs.readFile(filePath, (error, content) => {
            ...
            if (!query.source) {
                ...
                res.writeHead(200, { 'Content-Type': contentType });
                let cnt = Buffer.from(content.toString().replace(this.rxChart, JSON.stringify(this.demoChart())));
            ...
            switch (query.source) {
                    ...
                    this.adapter.sendTo(
                        ...
                        result => {
                            if (result.error) {
                                ...
                                res.writeHead(200, { 'Content-Type': contentType });
                                let cnt = Buffer.from(
                                    content.toString().replace(this.rxChart, JSON.stringify(this.demoChart())),
                                );
                                cnt = this.applyDarkmode(cnt, queryDM);
                                res.end(cnt, 'utf-8');
                ...
                default: {
                    res.writeHead(200, { 'Content-Type': contentType });
                    let cnt = Buffer.from(
                        content.toString().replace(this.rxChart, JSON.stringify(this.demoChartGauge())),
                    );
                    cnt = this.applyDarkmode(cnt, queryDM);
                    res.end(cnt, 'utf-8');
                    break;
