...
const DEFAULT_REQUEST_TIMEOUT = 2000; // Default timeout (ms) for sendTo() calls to javascript.0, overridable via query parameter requestTimeout
...
class ExtensionFlexcharts {
    ...
    /**
     * Parses the optional requestTimeout query parameter used for sendTo() calls to javascript.0.
     * Falls back to DEFAULT_REQUEST_TIMEOUT if the parameter is missing or not a positive number.
     *
     * @param {object} query Parsed query parameters of the request
     * @returns {number} Timeout in milliseconds
     */
    static getRequestTimeout(query) {
        const value = Number(query.requestTimeout);
        return Number.isFinite(value) && value > 0 ? value : DEFAULT_REQUEST_TIMEOUT;
    }
        ...
        switch (query.source) {
            case 'script': {
                ...
                this.adapter.sendTo(
                    ...
                    { timeout: ExtensionFlexcharts.getRequestTimeout(query) },
                );
                break;
            }
            case 'state': {
